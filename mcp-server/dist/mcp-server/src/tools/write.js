import { BulkUpdateColorInput, DeleteLayerInput, UpdateColorInput, UpdateDimensionsInput, UpdateFramerateInput, } from "../schemas.js";
import { deleteLayer, getSelectedShape, updateDimensions, updateFramerate, updateShapeColor, } from "../../../lib/animation.js";
...
export const register = (server, store) => {
    server.registerTool("update_shape_color", {
        description: "Set the color of a single shape (fill or stroke) at the given lodash path. Pushes a new entry onto the document's undo history.",
    ...
    server.registerTool("update_framerate", {
        description: "Set the framerate (frames per second) of the animation. Pushes onto undo history.",
    ...
    server.registerTool("update_dimensions", {
        description: "Set the canvas width and height of the animation. Pushes onto undo history.",
    ...
    server.registerTool("delete_layer", {
        description: "Remove the layer at the given index from the animation. Pushes onto undo history.",
    ...
    server.registerTool("bulk_update_color", {
        description: "Replace one color with another across every fill/stroke whose color is within `tolerance` (default 8) of `from`. A single undo entry is recorded for the whole bulk change.",
        inputSchema: BulkUpdateColorInput,
    }, async ({ doc_id, from, to, tolerance }) => {
        const id = store.resolveId(doc_id);
        const entry = store.get(id);
        const tol = tolerance ?? DEFAULT_TOLERANCE;
        const matches = walkColoredShapes(entry.history.current).filter((s) => colorDistance(s.color, from) <= tol);
