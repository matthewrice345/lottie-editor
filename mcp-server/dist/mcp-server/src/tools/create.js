...
import { AddPathInput, AddPolygonInput, AddPrimitiveInput, AddShapeLayerInput, AddStarInput, AddStrokeInput, AddTextLayerInput, ImportImageInput, ImportSvgInput, } from "../schemas.js";
import { addPath, addPolygonShape, addShape, addShapeLayer, addStarShape, addStroke, addTextLayer, importImageLayer, importSvgLayers, } from "../../../lib/animation.js";
import { readImageFile } from "../import-image.js";
import { readSvgFile } from "../import-svg.js";
import { jsonResult } from "./shared.js";
export const register = (server, store) => {
    server.registerTool("add_shape_layer", {
        description: "Append a new empty shape layer to the document. The layer is positioned at the canvas center with an identity transform and an out-frame matching the animation's existing duration. Pushes onto undo history.",
    ...
    server.registerTool("add_rectangle", {
        description: "Append a filled rectangle (wrapped in a Group with a Fill and Transform) to the shapes array of an existing shape layer. Defaults: 200×200, position [0,0]. Returns the new shape's path. Pushes onto undo history.",
        ...
        store.applyMutation(id, next);
        const newShapeIndex = next.layers[layer_index].shapes.length - 1;
        return jsonResult({
            doc_id: id,
            shape_path: `layers.${layer_index}.shapes.${newShapeIndex}`,
        });
    });
    server.registerTool("add_ellipse", {
        description: "Append a filled ellipse (wrapped in a Group with a Fill and Transform) to the shapes array of an existing shape layer. Defaults: 200×200, position [0,0]. Returns the new shape's path. Pushes onto undo history.",
        inputSchema: AddPrimitiveInput,
    }, async ({ doc_id, layer_index, color, width, height, position, name }) => {
        const id = store.resolveId(doc_id);
        const entry = store.get(id);
    ...
    server.registerTool("import_image", {
        description: "Import a PNG or JPEG file from disk into the lottie as a new image layer. The file is read, dimensions are detected, and the bytes are embedded base64 in the lottie's `assets` array (so the image travels with the file on save). The new layer is positioned at canvas center with identity transform. Note: image layers are unitary — you can move/scale/rotate/animate the whole image but cannot decompose it into pieces. For SVGs that need decomposition, use `import_svg` (when available). Pushes onto undo history.",
    ...
    server.registerTool("add_polygon", {
        description: "Append a regular polygon (triangle, square, pentagon, hexagon, etc.) to a shape layer. Uses Lottie's native `sr` polystar shape with `sy:2`. `points` is the number of sides (>= 3). `outer_radius` is the distance from center to vertex. Pushes onto undo history.",
    ...
    server.registerTool("add_star", {
        description: "Append a star (or starburst) to a shape layer. Uses Lottie's native `sr` polystar shape with `sy:1`. `points` is the number of star points; `outer_radius` and `inner_radius` (default outer/2) define the spikiness. `outer_roundness` and `inner_roundness` round the corners (0–100). Pushes onto undo history.",
        inputSchema: AddStarInput,
    }, async ({ doc_id, layer_index, points, outer_radius, inner_radius, rotation, outer_roundness, inner_roundness, position, color, name, }) => {
        const id = store.resolveId(doc_id);
        const entry = store.get(id);
    ...
    server.registerTool("add_path", {
        description: "Append a custom path (free-form polygon or bezier curve) to a shape layer. `vertices` is an array of [x, y] points in layer-local coords. Optional `in_tangents`/`out_tangents` (same length as vertices, [0,0] for sharp corners) define bezier curves. `closed` (default true) closes the path back to the first vertex. Optional `fill_color` / `stroke_color` / `stroke_width`. Pushes onto undo history.",
        inputSchema: AddPathInput,
    }, async ({ doc_id, layer_index, vertices, in_tangents, out_tangents, closed, fill_color, stroke_color, stroke_width, position, name, }) => {
        const id = store.resolveId(doc_id);
        const entry = store.get(id);
    ...
    server.registerTool("add_text_layer", {
    ...
    }, async ({ doc_id, text, font_family, font_size, color, position, justification, name, tracking, line_height, }) => {
        const id = store.resolveId(doc_id);
        const entry = store.get(id);
    ...
    server.registerTool("add_stroke", {
        description: "Add a stroke (outline) to an existing Group shape. The stroke is inserted before the Group's Transform so it renders correctly. Default width: 4. Pushes onto undo history.",
