...
const parsePngDimensions = (buf: Buffer): { width: number; height: number } => {
...
export const readImageFile = async (path: string): Promise<ImportedImage> => {
  if (!path.startsWith("/")) {
  ...
  if (ext === ".png") mime = "image/png";
    ...
    throw new Error(
      `Unsupported image extension: ${ext}. Supported: .png, .jpg, .jpeg`,
    );
