...
export const register = (server: McpServer, store: DocumentStore) => {
  server.registerTool(
    ...
    {
      description:
        "Load a lottie file from an absolute filesystem path or an http(s):// URL. Returns a doc_id used by subsequent tools, plus a one-shot summary of the animation.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Create a fresh empty lottie document with no layers and become its owner. Defaults: 1080×1080, 30fps, 3-second duration, name 'Untitled'. Returns a doc_id ready for `import_svg`, `add_shape_layer`, or other create tools. Use this as the first step when starting from scratch (e.g., when building a lottie around an SVG).",
  ...
  server.registerTool(
    ...
    {
      description:
        "Write the current state of a document to disk as JSON. If `path` is omitted, writes back to the document's original source path (only valid for documents loaded from a local file).",
    ...
    async ({ doc_id, path }) => {
      ...
      if (!target) {
        throw new Error(
          "No path provided and document has no source path (loaded from URL?). Provide an absolute `path`.",
        );
      }
      const bytes = await writeToPath(target, entry.history.current);
      if (!entry.sourcePath) entry.sourcePath = target;
      store.markClean(id);
  ...
  server.registerTool(
    ...
    {
      description:
        "Drop a document from memory. Warns if there are unsaved changes (returns dirty=true) but still closes.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Get a single-call overview of a document: dimensions, framerate, duration, layer count, and unique color palette.",
