...
export const register = (server: McpServer, store: DocumentStore) => {
  server.registerTool(
    ...
    {
      description:
        "Set transform properties on a Group shape (the dual of `set_layer_transform`, but for shapes inside a layer). `position` is [x, y] in layer-local pixels. `scale` is a percentage (100 = identity). `rotation` in degrees. `opacity` 0–100. Omitted properties are left unchanged. Pushes onto undo history.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Compute the bounding box of a layer's content. Returns both `layer_local` (in the layer's own coordinate space, useful for setting group transforms) and `canvas` (in canvas pixels, useful for aligning to the page). Best-effort: exact for shape layers built from rectangles/ellipses/groups; returns approximations for paths and unsupported types.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Compute the bounding box of a single shape (group or primitive) at the given lodash path. Returns layer-local and canvas coordinates plus a best-effort flag.",
  ...
  server.registerTool(
    ...
    {
      description:
        "High-level positioning: align a layer or group to a canvas-relative slot in one call. `target_path` is a layer path (`layers.N`) or group path (`layers.N.shapes.M`). Horizontal: left|center|right|third-left|third-right. Vertical: top|middle|bottom|third-top|third-bottom. `margin` (default 0) is the offset from the canvas edge for left/right/top/bottom anchors. Internally uses get_*_bounds to figure out the object's size, then nudges the appropriate transform. Pushes one undo entry.",
    ...
    async ({ doc_id, target_path, horizontal, vertical, margin }) => {
      ...
      if (!isLayerPath && !isShapePath) {
        throw new Error(
          `target_path must be a layer (layers.N) or shape path (layers.N.shapes.M…); got "${target_path}"`,
        );
      }
      ...
      if (!bounds.canvas) {
        throw new Error(
          `Could not compute bounds for ${target_path}. Notes: ${bounds.notes.join("; ")}`,
        );
      }
      ...
      if (isLayerPath) {
        ...
        const groupNode = (
          await import("../../../lib/animation.js")
        ).getSelectedShape(animation, groupPath);
        if (!groupNode) {
          throw new Error(`No shape at ${groupPath}`);
        }
        // Read current group tr.p
        const lodash = await import("lodash-es");
        const itArr = lodash.get(animation, `${groupPath}.it`) as
          | unknown[]
          | undefined;
        const tr = itArr?.find((s) => (s as { ty?: string }).ty === "tr") as
          | { p?: { k?: number[] } }
          | undefined;
        if (!tr) throw new Error(`Group at ${groupPath} has no transform`);
