...
export const register = (server: McpServer, store: DocumentStore) => {
  ...
  server.registerTool(
    ...
    {
      description:
        "Make one layer a child of another. The child's transform inherits from the parent's, so moving the parent moves the child too. Pass `parent_layer_index: null` to detach. Foundational for character rigging and compound motion.",
      inputSchema: SetLayerParentInput,
    },
    async ({ doc_id, child_layer_index, parent_layer_index }) => {
      const id = store.resolveId(doc_id);
      const entry = store.get(id);
      const next = setLayerParent(
  ...
  server.registerTool(
    ...
    {
      description:
        "Deep-clone a layer (including all its shapes, transforms, and animations) and append the copy to the end of the layer stack. Returns the new layer's index. Pushes onto undo history.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Set the corner radius of a rectangle shape. `shape_path` can point at a `rc` shape directly or at a `gr` group containing one. Pushes onto undo history.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Set a layer's blend mode (how it composites against layers below). Modes: normal, multiply, screen, overlay, darken, lighten, color-dodge, color-burn, hard-light, soft-light, difference, exclusion, hue, saturation, color, luminosity.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Make a stroke shape dashed. `dash` is the dash length in pixels; `gap` defaults to the same value (so equal dashes/gaps); `offset` shifts the pattern start. Useful for line-drawing animations (animate the offset to draw the line on). Pushes onto undo history.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Add a drop shadow effect to a layer. Defaults: black, 100% opacity, 135° direction, 10px distance, 5px softness. Pushes onto undo history.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Add a Gaussian blur effect to a layer. `amount` is the blurriness in pixels. `dimensions`: 1 = horizontal & vertical (default), 2 = horizontal only, 3 = vertical only. Pushes onto undo history.",
