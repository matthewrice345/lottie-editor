...
export const register = (server: McpServer, store: DocumentStore) => {
  server.registerTool(
    "get_animation_layers",
    {
      description:
        "Get the full layer tree for a document. Each layer contains shape descendants with lodash-style paths (e.g. `layers.0.shapes.1`) usable with `get_shape` and `update_shape_color`.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Get details for a single shape by its lodash path (e.g. `layers.0.shapes.1`).",
  ...
  server.registerTool(
    "find_shapes_by_color",
    {
      description:
        "Find all fill/stroke shapes whose color is within `tolerance` (Euclidean distance in 0–255 RGB space, default 8) of the target color. Returns layer/shape paths usable with `update_shape_color`.",
      inputSchema: FindByColorInput,
    },
    async ({ doc_id, color, tolerance }) => {
      const a = store.get(store.resolveId(doc_id)).history.current;
      const tol = tolerance ?? DEFAULT_TOLERANCE;
      const matches = walkColoredShapes(a).filter(
        (s) => colorDistance(s.color, color) <= tol,
      );
  ...
  server.registerTool(
    ...
    {
      description:
        "Find layers whose name matches `query`. Modes: `exact` (default), `contains` (substring, case-insensitive), `regex` (full JS regex).",
